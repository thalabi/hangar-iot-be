...
public class KeycloakJwtRolesConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
	...
	public Collection<GrantedAuthority> convert(Jwt jwt) {
		...
		if (! /* not */ CollectionUtils.isEmpty(realmAccess)) {
			...
			if (! /* not */ CollectionUtils.isEmpty(roles)) {
		...
		if (! /* not */ CollectionUtils.isEmpty(resourceAccess)) {
			...
			resourceAccess.forEach((resource, resourceClaims) -> {
				...
				resourceClaims.get(CLAIM_ROLES).forEach(
						...
						role -> grantedAuthorities
								.add(new SimpleGrantedAuthority(PREFIX_RESOURCE_ROLE + resource + "_" + role)));
			});
		}
	...
	public String getUsername(Jwt jwt) {
		LOGGER.info(jwt.getClaim(usernameAttribute));
